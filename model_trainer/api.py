...
def load_cluster_config():
    ...
    memory_worker_limit = cluster.get("memory_worker_limit", "1G")
    return number_workers, cpu_worker_limit, memory_worker_limit
...
def loadshifting():
    ...
    number_workers, cpu_worker_limit, memory_worker_limit = load_cluster_config()
    data_settings['file_name'] = experiment_name
    data_source = 's3'
    task = "load_shifting"
    ray_handler = RayKubeJobHandler()
    try:
        task_id = ray_handler.start_job(task, None, user_id, experiment_name, data_settings, ray_image, toolbox_version, data_source, number_workers, ray_version, cpu_worker_limit, memory_worker_limit)
...
def mlfit():
    ...
    ray_handler = RayKubeJobHandler()
    try:
        task_id = ray_handler.start_job(task, task_settings, user_id, experiment_name, data_settings, ray_image, toolbox_version, data_source, number_workers, ray_version, cpu_worker_limit, memory_worker_limit)
        return jsonify({'task_id': str(task_id), 'status': 'Processing'})
    except Exception as e:
        current_app.logger.error("Could not start job: " + str(e))
